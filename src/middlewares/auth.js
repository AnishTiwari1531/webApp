...
const authentication = async function (req, res, next) {
    try {
        ...
        jwt.verify(token, "CustomerWebApp", function (err, decoded) {
            if (err) {
...
const authorisation = async function (req, res, next) {     //userId from params
    try {
        ...
        req.id = loggedInUserId;
        if (loggedInUserId !== userId) return res.status(403).send({ status: false, message: `customer ${loggedInUserId} is not authorised to make changes in ${userId}` });
        next();
